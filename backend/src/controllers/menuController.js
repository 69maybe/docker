...
const mapMenuItem = (row) => ({
  ...
  image: row.hinh_anh,
  isActive: Boolean(row.trang_thai),
...
exports.createMenuItem = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO mon_an (ten_mon, mo_ta, gia, hinh_anh, trang_thai, loai_mon, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [name, description || null, price, imageUrl, isActive ? 1 : 0, category, status]
    )
...
exports.updateMenuItem = async (req, res, next) => {
  try {
    ...
    const payload = {
      ...
      gia: typeof price === 'undefined' ? existingRows[0].gia : price,
      hinh_anh: imageUrl,
      trang_thai: typeof isActive === 'undefined' ? existingRows[0].trang_thai : (isActive ? 1 : 0),
    ...
    await pool.query(
      `UPDATE mon_an 
       SET ten_mon = ?, mo_ta = ?, gia = ?, hinh_anh = ?, trang_thai = ?, loai_mon = ?, status = ? 
       WHERE id = ?`,
      [payload.ten_mon, payload.mo_ta, payload.gia, payload.hinh_anh, payload.trang_thai, payload.loai_mon, payload.status, id]
    )
